...
def load_dataset(benchmark, datapath, thres, device, split='test',img_side=None):
    ...
    return dataset(benchmark, datapath, thres, device, split,img_side=img_side)
...
def download_dataset(datapath, benchmark):
    ...
    file_data = {
        ...
        'spair-mini': ('1s73NVEFPro260H1tXxCh1ain7oApR8of', 'SPair-71k')
    }

    file_id, filename = file_data[benchmark]
    abs_filepath = os.path.join(datapath, filename)
    print('debug abs_filepath: exist {}, {}'.format(os.path.exists(abs_filepath),abs_filepath))
